import sss_parse as sss
import netswitch_parse as switch
...
def service(serv_name, serv_des):
    #service = input('Услуга(ШПД-1, телефония-2, Хот-спот-3, ЦКС-4, ВЛС-5):  ')

    if 'Интернет' in serv_name:          #service == '1':
        masks = ['/29', '/28']
        #access = input('access/trunk(a,t): ')
        #if access == 'a':
        if serv_name == 'Интернет, DHCP':
    ...
    elif 'Телефон' in serv_name:                  #service == '2':
    ...
    elif 'HotSpot' in serv_name:      #service == '3':
    ...
    elif 'iTV' in serv_name:
        #if 'Интернет, DHCP' in serv_name: 
        result = itv32.format(num=len(titles)+1)
#        elif 'Интернет, блок Адресов Сети Интернет' in serv_name:
#            marsh = input('iTV через марш или отдельно? ')
#            if marsh in yes: 
...
sservices_full, sservices, new_AB, turnoff, sreda = sss.matching(sss.input_TR())
print(sservices_full, sservices, new_AB)
...
for serv_name in sservices: #range(kol):
    for serv_des in sservices_full:
        if serv_name in serv_des:
            usl, usl_tit, titles, csw_ports_tag = service(serv_name, serv_des)
            serv.append(usl)
            title_serv.append(usl_tit)


res_tag = []
for tag in csw_ports_tag:
    res_tag.append('Port {numb} - "{tag}"'.format(numb=csw_ports_tag.index(tag)+1, tag=tag))
