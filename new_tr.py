...
port_vm = '''{num}. Организация услуги порт ВМ access'ом
------------------------------------------------------------------------------------------

ОНИТС СПД проведение работ:
- По заявке в Cordis организовать клиенту услугу порт виртуального маршрутизатора без доступа в интернет для ВМ {vm_name}, с полосой {vm_rate} Мбит/с, в порт подключения выдать vlan access.
- Адресацию согласовать на этапе реализации услуги.'''
...
def service(serv_des, sservices):
    ...
    if 'Интернет' in serv_des:
        ...
        vm_name = input('Название ВМ: ')
        vm_rate = input('Скорость: ')
        result = port_vm.format(num=len(titles)+1, vm_name=vm_name, vm_rate=vm_rate)
        title = result[:result.index('---')]
        titles.append(title)
        csw_ports_tag.append('порт ВМ')
